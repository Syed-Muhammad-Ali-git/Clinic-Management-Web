      ...
      {isLoading ? (
        ...
        <div className="text-center py-16 text-gray-400 bg-white rounded-xl border border-gray-200">
          ...
          <p className="text-sm mt-1">
            {search ? 'Try a different search term.' : 'No appointments scheduled yet.'}
          </p>
        </div>
      ) : (
                  ...
                  <tr key={a.id} className="hover:bg-gray-50 transition">
                    <td className="px-4 py-3 font-medium text-gray-900">
                      <div className="flex items-center gap-2">
                        <span className="w-7 h-7 rounded-full bg-cyan-100 text-cyan-600 flex items-center justify-center text-xs font-bold">
                          {(a.patientName || a.patientId || '?')[0].toUpperCase()}
                        </span>
                    ...
                    <td className="px-4 py-3">
                      {canManage ? (
                        ...
                        <span className={`text-xs px-2 py-1 rounded-full font-medium capitalize ${STATUS_COLORS[a.status] || 'bg-gray-100 text-gray-600 border border-gray-200'}`}>
                          {a.status}
                        </span>
                      )}
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-2">
                        ...
                        {canManage && (
                          <button
                            onClick={() => handleDelete(a.id, a.patientName || a.patientId || 'this patient')}
